...
for i = 1:size(col_vec,1)
    if col_vec(i,1) - i < .5
        sprintf('%.3f Less Than', col_vec(i,1))
    elseif col_vec(i,1) - i == .5
        sprintf('%.3f Equal', col_vec(i,1))
    elseif col_vec(i,1) - i > .5
        sprintf('%.3f Greater Than', col_vec(i,1))
    end
end
...
fprintf('#subjects: %d', size(n_subjects,1))
...
fprintf('#factors: %d', size(n_factors,1))
